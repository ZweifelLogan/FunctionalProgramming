...
instance Applicative Logging where
  pure v = Logging [] v --Logging undefined v
...
rlelh c h [] = lg "base" <*> pure [(c, h)] --reached end of input from rlel
rlelh c h' (h:t) = 
  let r = rlelh 1 h t in
    if h == h' then
      rlelh (c+1) h t
    else
      fmap (++) (lg "change" <*> pure [(c,h')]) <*> r
    -- that gets through the rest of the list
